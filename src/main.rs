...
/// draws a greeting.
fn ui(frame: &mut ratatui::Frame<CrosstermBackend<Stdout>>) {
    let size = frame.size();
    let layout = Layout::default().direction(Direction::Vertical).constraints(vec![Constraint::Min(1), Constraint::Length(3)]).split(size);
